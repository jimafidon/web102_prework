...
const totalRaisedCard = GAMES_JSON.reduce( (acc, pledge) => {
...
function filterUnfundedOnly() {
    ...
    const notReachedGoal = GAMES_JSON.filter( (game) => {
...
function filterFundedOnly() {
    ...
    const reachedGoal = GAMES_JSON.filter( (game) => {
        return game.pledged >= game.goal;
    });
...
const displayStr = 
    `A total of $${totalRaisedCard.toLocaleString()} has been raised for 
    ${(totalGames)} games. 
...
let topPledgeName = document.createElement('p');
topPledgeName.innerHTML = first.name;
